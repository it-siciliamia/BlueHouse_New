...
const CalendarDays = () => {
    ...
    const handleDateClick = useCallback((actualDate) => {
        console.log('tsss')
        if (status === 'start') {
    ...
    }, [status, selectedStart, selectedEnd, setSelectedEnd, setSelectedStart, voidSelectedEnd, setStatus])

    const getDateStatuses = useCallback((date) => {
    ...
    return (
        <div className={`calendar-popup fixed top-0 left-0 right-0 calendar-el pb-8 border-8 px-4 h-[40%] w-full max-w-[325px] overflow-y-scroll overflow-x-hidden bg-white text-black`}
                    ...
                    <div key={month}>
                        ...
                        <ul className="grid grid-cols-7 gap-1 list-none">
                            {days.map((day, index) => {
                                ...
                                const actualDate = new Date(selectedYear, monthIndex[month], Number(day))
                                ...
                                return (
                                    <li
                                        ...
                                        className={`py-0.5 px-1 cursor-pointer text-center
                                                    ...
                                                    ${isStart ? 'bg-marine text-white' : ''}
                                                    ${isEnd ? 'bg-marine text-white' : ''}
                                    ...
                                    >
                                        <div className="flex justify-center items-center h-[100%]" id={actualDate}>
                                            ...
                                            {isToday(new Date(selectedYear, monthIndex[month], Number(day))) && <div className="border-2 p-1 border-[#d8e2dc]">{day}</div>}
