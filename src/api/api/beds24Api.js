...
async function ensureAuthentication() {
  ...
  const isTokenExpired =
    !tokenExpirationTime || now >= tokenExpirationTime - 300000;
  ...
  if (!authInitialized || isTokenExpired) {
    try {
      ...
      console.error(
        "Failed to initialize Beds24 authentication:",
        error.message
      );
      throw error;
    }
...
router.get("/properties", async (req, res) => {
  try {
    ...
    console.error(
      "Error fetching rooms with pictures:",
...
router.post("/bookings", async (req, res) => {
  try {
    ...
    res.json({ success: true, bookingid, data: response.data });
  } catch (error) {
    console.error(
