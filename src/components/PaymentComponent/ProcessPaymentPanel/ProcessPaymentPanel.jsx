    ...
    <div className={s.paymentPanel}>
      {stages.map((item, index) => (
        <React.Fragment key={item.stage}>
          <div className={`${s.circle} ${paymentStage >= item.stage ? s.active : ""}`}>
            <span style={{ marginTop: "3px" }}>{item.stage}</span>
            <span className={`${s.label} ${paymentStage >= item.stage ? s.active : ""}`}>
              <WithTransLate text={item.label} />
            </span>
          </div>
          {index < stages.length - 1 && (
            <div className={`${s.line} ${paymentStage > item.stage ? s.activeLine : ""}`}></div>
          )}
        </React.Fragment>
      ))}
    </div>
